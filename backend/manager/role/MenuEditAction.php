<?php

namespace backend\manager\role;

use backend\models\EmployeeRole;
use backend\models\ManagerMenuRule;
use backend\models\ManagerRole;
use backend\models\MenuRule;
use common\base\api\ApiAction;
use common\base\api\ManagerApiAction;
use common\exception\ApiException;
use Yii;
use yii\base\Exception;


/**
 * @Class MenuEditAction
 * @package backend\api\employeeRole
 * @User:五更的猫
 * @DateTime: 2023/8/15 11:48
 * @TODO 菜单权限编辑
 */
class MenuEditAction extends ManagerApiAction
{
    public $isLogin = true;
    protected function runAction()
    {
        $role_id  = $this->RequestData('role_id',0);
        $auth  = $this->RequestData('auth',array());

        if(empty($role_id)){
            throw new ApiException('请选择角色',1);
        }
        $model = ManagerRole::find()->andWhere(['id'=>$role_id])->one();

        if (empty($model)) {
            throw new ApiException('不存在此角色',1);
        }
        if(!is_array($auth)){
            $auth = explode(',',$auth);
        }

        $MenuChild = new ManagerMenuRule();

        $result = $MenuChild->setMenus($role_id,$auth);

        if($result != true) {
            throw new ApiException('菜单显示分配失败',1);
        }

        $jsonData['errmsg'] = '菜单显示分配成功';


        return $jsonData;
    }

}